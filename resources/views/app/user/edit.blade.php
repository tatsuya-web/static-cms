<x-app-layout title="編集 | 管理ユーザー">
    <header class="main_hd">
        <h1 class="main_hd_ttl mb-0">編集 | 管理ユーザー</h1>
    ...
    <div class="main_cnt">
        <form action="{{ route('app.user.update', ['user' => $user]) }}" method="post">
            @csrf
            <div class="grid">
                <div class="col-6">
                    <div class="card">
                        ...
                        <p>
                            入力項目に必要事項を入力してくし、「更新する」ボタンをクリックすることで編集が完了します。
                        </p>
                        <div class="col-12">
                            <div class="input @error('name') -invalid @enderror">
                                <label class="input_ttl -req">ユーザー名</label>
                                <input type="text" name="name" value="{{ old('name', $user->name) }}" placeholder="ユーザー名を入力してください" required>
                                @error('name')
                            ...
                            <div class="input @error('email') -invalid @enderror"> 
                                ...
                                <input type="email" name="email" value="{{ old('email', $user->email) }}" placeholder="emailを入力してください" required>
                                @error('email')
                                <div class="input_feedback">
                                    <span>{{ $message }}</span>
                                </div>
                            ...
                            <div class="input @error('role') -invalid @enderror">
                                ...
                                <select name="role">
                                    ...
                                    @foreach(\App\Enums\Role::cases() as $role)
                                        <option value="{{ $role }}" @if(old('role', $user->role) == $role) selected @endif>{{ $role->getNameJa() }}</option>
                                    @endforeach
                                </select>
                                @error('role')
                                <div class="input_feedback">
